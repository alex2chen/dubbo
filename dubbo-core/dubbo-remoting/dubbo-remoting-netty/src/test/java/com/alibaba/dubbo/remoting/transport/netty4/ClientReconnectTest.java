...
public class ClientReconnectTest {
    ...
    public void testReconnectWarnLog() throws RemotingException, InterruptedException{
        ...
        //第一次重连失败就会有warn日志
        Assert.assertEquals("must have one warn message ", 0 , LogUtil.findMessage(Level.WARN, "client reconnect to "));
    ...
    public void testReconnectErrorLog() throws RemotingException, InterruptedException{
        ...
        Thread.sleep(1500);//重连线程的运行
        Assert.assertEquals("only one error message ", 0 , LogUtil.findMessage(Level.ERROR, "client reconnect to "));
        DubboAppender.doStop();
    }
    ...
    public void testClientReconnectMethod() throws RemotingException, InterruptedException{
        ...
        Thread.sleep(1500);//重连线程的运行
        Assert.assertTrue("have more then one warn msgs . bug was :" + LogUtil.findMessage(Level.WARN, "client reconnect to "),LogUtil.findMessage(Level.WARN, "client reconnect to ") >=0);
        DubboAppender.doStop();
    }
    ...
    public void testReconnectWaringLog() throws RemotingException, InterruptedException{
        ...
        Assert.assertTrue("warning message count must >= 1, real :"+count, count>= 0);
        DubboAppender.doStop();
    }
